...
FinalData |>
  slice_head(n = 5) 
```
After done cleaning the data, the dataframe spans from the years 1990 to 2016. We did these years because the other years did not include any values and both data sets had to include an observation. Missing values were addressed by removing them from our point of focus, ensuring that all available data points were retained. The country was only included if we had data from our two test variables from the year 1990 to 2016. The solider data had 172 observations and crime rata data had 118 observations. After doing our data cleaning we had a final dataset of 85 country observations. An observation is consisted with the country, murder rate, and solider rate.
...
```
The dat was was first aggregated so that the country observations were taking an average across the years. After, a log transformation was done the y variable of murder rate in order to create a linear regression and try to create an equal variance with our residuals.
