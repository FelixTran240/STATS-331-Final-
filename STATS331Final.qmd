...
ggplot() +
  geom_point(data = summary_data, aes(x = avg_soldiers, y = avg_murder_rate), color = "blue", alpha = 0.5) +
  geom_point(data = simulated_data, aes(x = avg_soldiers, y = simulated_murder_rate), color = "red", alpha = 0.5) +
...
Thousand_Simulated_id <- Thousand_Simulated |>
  mutate(simulation_id = (row_number() - 1) %/% 96 + 1)
...
r_squared_values <- map_dbl(1:1000, function(i) {
  ...
  sub_model <- lm(avg_murder_rate ~ simulated_murder_rate, data = merged_data)
